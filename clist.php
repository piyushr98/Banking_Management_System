 ...
 <body style="background-image: url(images/123.jpg); background-size: cover; background-position: center;">
	<!-- Top-Bar -->
		...
		<marquee><h2>CORPORATE BANK</h2></marquee>
						...
						<tr>
							<th color="white"><h4>Account no</h4></th>
							<th color="white"><h4>Name</h4></th>
							<th color="white"><h4>Email</h4></th>
							<th color="white"><h4>Dob</h4></th>
							<th color="white"><h4>Gender</h4></th>
							<th color="white"><h4>Registration Date</h4></th>
							<th color="white"><h4>Balance</h4></th>
						</tr>
						<tr>
...
$conn = mysqli_connect("localhost", "root", "", "banking");
  ...
  $sql = "SELECT * from cust_list";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
   // output data of each row
   while($row = $result->fetch_assoc()) {
   	
    echo "<tr><td>" . $row["accno"] . "</td><td>"
. $row["name"]. "</td><td>" . $row["email"] . "</td><td>"
. $row["dob"]. "</td><td>" . $row["gender"] . "</td><td>"
. $row["time"]. "</td><td>" . $row["balance"] . "</td></tr>";
